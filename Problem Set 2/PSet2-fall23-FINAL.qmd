...
author: "Alexandra Kakadiaris and Sammy She"
execute:
  echo: true
  warning: false
format:
...
# For this part, we will divide the release year into equal range groups (similar to binwidths), namely decade periods. Use cut() to create a new variable called decade to represent the decades: 1950-1959, 1960-1969, etc. Redraw the graph from part a) faceting on decade. What trends do you observe?
...
subset_movies_df <- subset_movies_df %>% 
  mutate(period = recode(period, "Q1" = "1955-2004", 
                                 "Q2" = "2005-2012", 
                                 "Q3" = "2013-2016", 
                                 "Q4" = "2017-2021"))
...
subset_movies_df <- subset_movies_df %>% mutate(period2 = recode(period2, "1" = "1955-2005", "2" = "2005-2013", "3" = "2013-2017", "4" = "2017-2021"))
...
One of the abnormalities that is present is that safflower seed oil, olive oil, and corn oil all hvae the same calorie density. Another anomaly is the fact that this also happens between thousand island sauce and powdered milk.
...
#combine vegetables and fruits columns 
Vegetables <- data.frame(Category = "Vegetables", mean_calorie_density = 1.801347)
Fruits <- data.frame(Category = "Fruits", mean_calorie_density = 2.686081)

# Create a new row as a data frame
food_categories <- rbind(food_categories, Vegetables)
food_categories <- rbind(food_categories, Fruits)

# Remove the segmented Vegetables and Fruits rows
values_to_remove <- c("Vegetables F-P","Vegetables A-E","Vegetables R-Z","Fruits A-F","Fruits R-Z","Fruits G-P")
food_categories <- food_categories[!(food_categories$Category %in% values_to_remove), ]
...
Abnormalities: We decided to combine all the vegetable and fruits row to come up with this analysis. (Outlier) there are two dots for the same butter, clearly the smaller one is an outlier. From nutrients data set, there are three Butter; two of them have calorie_density around 1, one is around 7.4.
...
g <- ggplot(nutrients_protein_calories, aes(Calories, protein_calories, label = Food)) +
  geom_point(color = "cornflowerblue")
ggplotly(g)
...
Density contour lines clearly show the density in the clustered region, suggesting a strong relationship. Gestation and bwt are more likely to have combination pairs around 250-300 gestation in days, and 80-160 bwt in ounces. The highest concentration seems to be around 275 gestation and 121 bwt.

c)  Hexagonal heatmap of bin counts

```{r}
ggplot(babies, aes(y = bwt, x = gestation)) +
  geom_hex(binwidth = c(5, 10)) +
...
Hexagonal heatmap of bin counts uses bins to represent data points. We can see the clear number counts distinguished by colors. The most concentrated part has over 30 counts, which means over 30 data points cluster at around 275 gestation and 121 bwt. The concentration region decreases from over 30 counts to less than 10 counts, and the heatmap explicitly shows outliers.
